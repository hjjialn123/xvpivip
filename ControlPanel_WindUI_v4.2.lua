...
Tabs.Visual:Dropdown({
    Title = "选择滤镜",
    Values = { "Normal", "Warm", "Cool", "Vintage", "Cyber", "Mono", "Night", "Dream" },
    Value = "Normal",
...
--  【在这里添加彩色视角代码】
-- ============================================
Tabs.Visual:Section({ Title = "彩色视角" })

-- 彩色视角状态变量（放在脚本顶部的State区域）
-- State.colorViewRunning = false
-- State.colorViewSpeed = 0.2
...
Tabs.Visual:Slider({
    ...
    Callback = function(value)
        colorViewSpeed = value / 40 + 0.025
        if colorViewRunning then
            if colorViewConnection then
                colorViewConnection:Disconnect()
                colorViewConnection = nil
            end
            local wasRunning = colorViewRunning
            colorViewRunning = false
            task.wait(0.05)
            toggleColorView(wasRunning)
        end
    end
})
-- 色彩滤镜
...
local function applyColorFilter(filterType)
    ...
    local presets = {
        ...
        Red = { TintColor = Color3.fromRGB(255, 0, 0), Contrast = 1, Saturation = 3, Brightness = 1 },
        Green = { TintColor = Color3.fromRGB(0, 255, 0), Contrast = 1, Saturation = 3, Brightness = 1 },
        Blue = { TintColor = Color3.fromRGB(0, 0, 255), Contrast = 1, Saturation = 3, Brightness = 1 },
        Saturation = { TintColor = Color3.fromRGB(0, 0, 0), Contrast = 0, Saturation = 10, Brightness = 1 },
-- 随机传送
